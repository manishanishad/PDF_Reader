...
public class PDSElementViewer {
    ...
    public void rotateElement() {
        if (this.mElementView.getParent() != null) {
            this.mElementView.setRotation(this.mElementView.getRotation() + 90);
        }
    }
    ...
    private View createElementView(PDSElement fASElement) {
        switch (fASElement.getType()) {
            ...
            case PDSElementTypeImage:
                ...
                createResizeButton();
                return imageView;
            case PDSElementTypeEditText:
                TextView textView = ViewUtils.createEditTextView(this.mContext, fASElement, this.mPageViewer.getToViewCoordinatesMatrix());
                textView.setText(fASElement.getText());
                fASElement.setRect(new RectF(fASElement.getRect().left, fASElement.getRect().top, fASElement.getRect().left + this.mPageViewer.mapLengthToPDFCoordinates((float) textView.getWidth()), fASElement.getRect().bottom));
                textView.setFocusable(true);
                textView.setFocusableInTouchMode(true);
                textView.setClickable(true);
                textView.setLongClickable(true);
                textView.setHint(R.string.text_here);
                textView.setTextColor(Integer.parseInt(PreferencesManager.Companion.getString(mContext, PreferencesManager.TEXT_COLOR)));
    ...
    private void createResizeButton() {
        ...
        this.mImageButton.setImageResource(R.drawable.ic_resize_view);
    ...
    public void showBorder() {
        ...
        gradientDrawable.setStroke(4, this.mContext.getResources().getColor(R.color.dark_grey), 10f, 4f);
    ...
    public void changeColor(boolean z) {
        int color = z ? this.mContext.getResources().getColor(R.color.red) : ViewCompat.MEASURED_STATE_MASK;
        if (this.mElementView instanceof SignatureView) {
