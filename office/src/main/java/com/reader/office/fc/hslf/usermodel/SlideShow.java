...
package com.reader.office.fc.hslf.usermodel;
...
import com.reader.office.fc.hslf.model.Hyperlink;
import com.reader.office.fc.hslf.model.MovieShape;
...
import com.reader.office.fc.hslf.record.PositionDependentRecord;
import com.reader.office.fc.hslf.record.PositionDependentRecordContainer;
import com.reader.office.fc.hslf.record.Records;
import com.reader.office.fc.hslf.record.RecordContainer;
import com.reader.office.fc.hslf.record.RecordTypes;
import com.reader.office.fc.hslf.record.SlideListWithText;
import com.reader.office.fc.hslf.record.SlidePersistAtom;
import com.reader.office.fc.hslf.record.ExtendedPresRuleContainer.ExtendedParaAtomsSet;
import com.reader.office.fc.hslf.record.SlideListWithText.SlideAtomsSet;
...
public final class SlideShow
{
    ...
    private void findMostRecentCoreRecords()
    {
        ...
        // the array
        _mostRecentCoreRecords = new Records[mostRecentByBytes.size()];
     ...
     */
    private Records getCoreRecordForSAS(SlideAtomsSet sas)
    {
     ...
     *            the refID
     */
    private Records getCoreRecordForRefID(int refID)
    {
        Integer coreRecordId = _sheetIdToCoreRecordsLookup.get(Integer.valueOf(refID));
        if (coreRecordId != null)
        {
            Records r = _mostRecentCoreRecords[coreRecordId.intValue()];
            return r;
        }
    ...
    private void buildSlidesAndNotes()
    {
        ...
        if (_documentRecord == null)
        {
            throw new CorruptPowerPointFileException(
                "The PowerPoint file didn't contain a Document Records in its PersistPtr blocks. It is probably corrupt.");
        }

        // Fetch the SlideListWithTexts in the most up-to-date Document Records
        //
        ...
        if (masterSLWT != null)
        {
            ...
            {
                Records r = getCoreRecordForSAS(masterSets[i]);
                SlideAtomsSet sas = masterSets[i];
                int sheetNo = sas.getSlidePersistAtom().getSlideIdentifier();
                if (r instanceof com.reader.office.fc.hslf.record.Slide)
                {
                    TitleMaster master = new TitleMaster((com.reader.office.fc.hslf.record.Slide)r, sheetNo);
                    master.setSlideShow(this);
                    tmr.add(master);
                }
                else if (r instanceof com.reader.office.fc.hslf.record.MainMaster)
                {
                    SlideMaster master = new SlideMaster((com.reader.office.fc.hslf.record.MainMaster)r, sheetNo);
                    master.setSlideShow(this);
                    mmr.add(master);
                }
            }
        ...
        // notesSLWT
        com.reader.office.fc.hslf.record.Notes[] notesRecords;
        SlideAtomsSet[] notesSets = new SlideAtomsSet[0];
        Hashtable<Integer, Integer> slideIdToNotes = new Hashtable<Integer, Integer>();
        if (notesSLWT == null)
        {
            // None
            notesRecords = new com.reader.office.fc.hslf.record.Notes[0];
        }
        else
        {
            // Match up the records and the SlideAtomSets
            notesSets = notesSLWT.getSlideAtomsSets();
            ArrayList<com.reader.office.fc.hslf.record.Notes> notesRecordsL = new ArrayList<com.reader.office.fc.hslf.record.Notes>();
            for (int i = 0; i < notesSets.length; i++)
            {
                // Get the right core record
                Records r = getCoreRecordForSAS(notesSets[i]);

                // Ensure it really is a notes record
                if (r instanceof com.reader.office.fc.hslf.record.Notes)
                {
                    com.reader.office.fc.hslf.record.Notes notesRecord = (com.reader.office.fc.hslf.record.Notes)r;
                    notesRecordsL.add(notesRecord);

                    // Records the match between slide id and these notes
            ...
            notesRecords = new com.reader.office.fc.hslf.record.Notes[notesRecordsL.size()];
            notesRecords = notesRecordsL.toArray(notesRecords);
        }

        // Now, do the same thing for our slides
        com.reader.office.fc.hslf.record.Slide[] slidesRecords;
        SlideAtomsSet[] slidesSets = new SlideAtomsSet[0];
        if (slidesSLWT == null)
        {
            // None
            slidesRecords = new com.reader.office.fc.hslf.record.Slide[0];
        }
        else
        {
            // Match up the records and the SlideAtomSets
            slidesSets = slidesSLWT.getSlideAtomsSets();
            slidesRecords = new com.reader.office.fc.hslf.record.Slide[slidesSets.length];
            for (int i = 0; i < slidesSets.length; i++)
            {
                // Get the right core record
                Records r = getCoreRecordForSAS(slidesSets[i]);

                // Ensure it really is a slide record
                if (r instanceof com.reader.office.fc.hslf.record.Slide)
                {
                    slidesRecords[i] = (com.reader.office.fc.hslf.record.Slide)r;
                }
                /*else
                {
     ...
     * records
     */
    public Records[] getMostRecentCoreRecords()
    {
    ...
    public void reorderSlide(int oldSlideNumber, int newSlideNumber)
    {
        ...
        ArrayList<Records> lst = new ArrayList<Records>();
        for (int i = 0; i < sas.length; i++)
        {
            lst.add(sas[i].getSlidePersistAtom());
            Records[] r = sas[i].getSlideRecords();
        ...
        Records[] r = lst.toArray(new Records[lst.size()]);
    ...
    public Slide removeSlide(int index)
    {
        ...
        else
        {
            ...
            slwt.setChildRecord(records.toArray(new Records[records.size()]));
        }
        ...
        if (removedSlide != null)
        {
            ...
            if (notesId != 0)
            {
                ...
                else
                {
                    ...
                    nslwt.setChildRecord(records.toArray(new Records[records.size()]));
                }
    ...
    public HeadersFooters getSlideHeadersFooters()
    {
        ...
        Records[] ch = _documentRecord.getChildRecords();
    ...
    public void dispose()
    {
        ...
        {
            for (Records rd : _mostRecentCoreRecords)
            {
                rd.dispose();
            }
    ...
    private Records[] _mostRecentCoreRecords;
