...
import com.reader.office.fc.ddf.EscherRecordFactory;
import com.reader.office.fc.ddf.EscherSerializationListener;
...
import com.reader.office.fc.hssf.model.AbstractShape;
import com.reader.office.fc.hssf.model.CommentShape;
import com.reader.office.fc.hssf.model.ConvertAnchor;
import com.reader.office.fc.hssf.model.DrawingManager2;
import com.reader.office.fc.hssf.model.TextboxShape;
import com.reader.office.fc.hssf.usermodel.HSSFChart;
import com.reader.office.fc.hssf.usermodel.HSSFClientAnchor;
import com.reader.office.fc.hssf.usermodel.HSSFPatriarch;
import com.reader.office.fc.hssf.usermodel.HSSFShape;
import com.reader.office.fc.hssf.usermodel.HSSFShapeContainer;
import com.reader.office.fc.hssf.usermodel.HSSFShapeFactory;
import com.reader.office.fc.hssf.usermodel.HSSFShapeGroup;
import com.reader.office.fc.hssf.usermodel.HSSFTextbox;
...
public final class EscherAggregate extends AbstractEscherHolderRecord {
	...
	private Map<EscherRecord, Records> shapeToObj = new HashMap<EscherRecord, Records>();
	//chart records
	 private Map<EscherRecord, List<Records>> chartToObj = new HashMap<EscherRecord, List<Records>>();
	...
	public static int shapeContainRecords(List records, int currentDrawingRecord)
	{
	    ...
	    {
	        Records record = (Records) records.get( currentDrawingRecord + 1 );            
	        count = 2;
	        //chart records
            if(record instanceof ObjRecord && ((ObjRecord)record).getSubRecords().get(0) instanceof CommonObjectDataSubRecord)
            {
                ...
                if(commonObjectDataSubRecord.getObjectType() == CommonObjectDataSubRecord.OBJECT_TYPE_CHART)
                {
                  //chart 
                    ...
                    record = (Records) records.get( currentDrawingRecord );
	...
	public static EscherAggregate createAggregate( List records, int locFirstDrawingRecord, DrawingManager2 drawingManager )
	{
		...
		loc = locFirstDrawingRecord;
		int shapeIndex = 0;
		agg.shapeToObj = new HashMap<EscherRecord, Records>();
		while ( loc > -1 && loc + 1 < records.size()
				&& (sid( records, loc ) == DrawingRecord.sid || sid( records, loc ) == ContinueRecord.sid))
		{
		    if(!isObjectRecord( records, loc + 1 ))
		    ...
		    Records record = (Records) records.get( loc + 1 );
		    try
		    {
		      //chart records
	            if(record instanceof ObjRecord && ((ObjRecord)record).getSubRecords().get(0) instanceof CommonObjectDataSubRecord)
	            {
	                ...
	                if(commonObjectDataSubRecord.getObjectType() == CommonObjectDataSubRecord.OBJECT_TYPE_CHART)
	                {
	                    ...
	                    List<Records> chartRecordsList = new ArrayList<Records>();
	                    loc += 2;     
	                    record = (Records) records.get( loc );
	                    while(record.getSid() != EOFRecord.sid)
	                    {
	                        chartRecordsList.add(record); 
	...
	public int serialize( int offset, byte[] data )
	{
		...
		for ( int i = 1; i < shapes.size(); i++ )
		{
			...
			Records obj = shapeToObj.get( shapes.get( i ) );
		...
		{
			Records rec = (Records)tailRec.get(i);
	...
	public int getRecordSize() {
		...
		{
			Records r = (Records) iterator.next();
	...
	private void convertRecordsToUserModel(EscherContainerRecord shapeContainer, Object model) 
	{
		for(Iterator<EscherRecord> it = shapeContainer.getChildIterator(); it.hasNext();) 
		{
			...
			else if(r instanceof EscherTextboxRecord) 
			{
				...
				Records obj = shapeToObj.get(tbr);
			...
			else if(r instanceof EscherClientDataRecord && model instanceof HSSFChart) 
            {
                ...
                List<Records> recordsList = chartToObj.get(cdr);
...
//		dg.setLastMSOSPID( 0 );  // populated after all shape id's are assigned.
	...
	{
		return ( (Records) records.get( loc ) ).getSid();
	}
}
