...
package com.reader.office.fc.hssf.eventmodel;
 ...
 * this version sends {@link ERFListener#processRecord(Records) } messages to
 * the supplied listener.  Records notifications are sent one record behind
...
public final class EventRecordFactory {
	...
	/**
	 *
	 * @param sids an array of Records.sid values identifying the records
	 ...
	 */
	private boolean processRecord(Records record) {
	...
	public void processRecords(InputStream in) throws RecordFormatException {
		...
		while (recStream.hasNextRecord()) {
			...
			Records[] recs = RecordFactory.createRecord(recStream);   // handle MulRK records
			if (recs.length > 1) {
