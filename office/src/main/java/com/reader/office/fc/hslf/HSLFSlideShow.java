...
package com.reader.office.fc.hslf;
...
import com.reader.office.fc.fs.filesystem.CFBFileSystem;
import com.reader.office.fc.fs.filesystem.Property;
import com.reader.office.fc.hslf.blip.Metafile;
...
import com.reader.office.fc.hslf.model.Picture;
import com.reader.office.fc.hslf.record.CurrentUserAtom;
import com.reader.office.fc.hslf.record.ExOleObjStg;
import com.reader.office.fc.hslf.record.PersistPtrHolder;
import com.reader.office.fc.hslf.record.PersistRecord;
import com.reader.office.fc.hslf.record.Records;
import com.reader.office.fc.hslf.record.UserEditAtom;
import com.reader.office.fc.hslf.usermodel.ObjectData;
...
public final class HSLFSlideShow/* extends POIDocument*/
{
    ...
    private Records[] read(/*byte[] docstream, */int usrOffset)
    {
        ArrayList<Integer> lst = new ArrayList<Integer>();
        HashMap<Integer, Integer> offset2id = new HashMap<Integer, Integer>();
        while (usrOffset != 0)
        {
//            UserEditAtom usr = (UserEditAtom)Records.buildRecordAtOffset(docstream, usrOffset);
            byte[] data = _docProp.getRecordData(usrOffset);
            UserEditAtom usr = (UserEditAtom)Records.buildRecordAtOffset(data, 0, usrOffset);
            
            lst.add(Integer.valueOf(usrOffset));
            int psrOffset = usr.getPersistPointersOffset();

//            PersistPtrHolder ptr = (PersistPtrHolder)Records.buildRecordAtOffset(docstream,
//                psrOffset);
            
            data = _docProp.getRecordData(psrOffset);
            PersistPtrHolder ptr = (PersistPtrHolder)Records.buildRecordAtOffset(data, 0, psrOffset);
        ...
        Arrays.sort(a);
        Records[] rec = new Records[lst.size()];
        for (int i = 0; i < a.length; i++)
        {
            Integer offset = a[i];
//            rec[i] = Records.buildRecordAtOffset(docstream, offset.intValue());
            byte[] data = _docProp.getRecordData(offset.intValue());
            rec[i] = Records.buildRecordAtOffset(data, 0, offset.intValue());
...
//                                out.write(pict.getData());
     ...
     */
    public synchronized int appendRootLevelRecord(Records newRecord)
    {
        int addedAt = -1;
        Records[] r = new Records[_records.length + 1];
        boolean added = false;
