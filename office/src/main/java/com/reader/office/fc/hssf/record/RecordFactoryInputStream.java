...
import com.reader.office.fc.hssf.eventusermodel.HSSFEventFactory;
import com.reader.office.fc.hssf.eventusermodel.HSSFListener;
import com.reader.office.fc.hssf.record.crypto.Biff8EncryptionKey;
...
public final class RecordFactoryInputStream {
	...
	private static final class StreamEncryptionInfo {
		...
		private final Records _lastRecord;
		private final boolean _hasBOFRecord;

		public StreamEncryptionInfo(RecordInputStream rs, List<Records> outputRecs) {
			Records rec;
			rs.nextRecord();
	...
	/**
	 * Temporarily stores a group of {@link Records}s, for future return by {@link #nextRecord()}.
	 ...
	 */
	private Records[] _unreadRecordBuffer;
	 ...
	 */
	private Records _lastRecord = null;
	/**
	...
	public RecordFactoryInputStream(InputStream in, boolean shouldIncludeContinueRecords) {
		...
		if (!records.isEmpty()) {
			_unreadRecordBuffer = new Records[records.size()];
	...
	/**
	 * @return the next {@link Records} from the multiple record group as expanded from
	 * a recently read {@link MulRKRecord}. <code>null</code> if not present.
	 */
	private Records getNextUnreadRecord() {
		if (_unreadRecordBuffer != null) {
			int ix = _unreadRecordIndex;
			if (ix < _unreadRecordBuffer.length) {
				Records result = _unreadRecordBuffer[ix];
	 ...
	 */
	private Records readNextRecord() {

		Records record = RecordFactory.createSingleRecord(_recStream);
		_lastRecordWasEOFLevelZero = false;
		...
		if (record instanceof MulRKRecord) {
			Records[] records = RecordFactory.convertRKRecords((MulRKRecord) record);
		...
		if (record.getSid() == ContinueRecord.sid) {
			...
			throw new RecordFormatException("Unhandled Continue Records followining " + _lastRecord.getClass());
		}
		_lastRecord = record;
