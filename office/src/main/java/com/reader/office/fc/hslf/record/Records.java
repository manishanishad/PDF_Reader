 ...
 * Records classes should extend with RecordContainer or RecordAtom, which
 *  extend this in turn.
 *
 * @author Nick Burch
 */
...
public abstract class Records
{
	 ...
	 *  an empty array
	 */
	public abstract Records[] getChildRecords();
	...
	/**
	 * Build and return the Records at the given offset.
	 ...
	 */
	public static Records buildRecordAtOffset(byte[] b, int offset) {
	...
	public static Records buildRecordAtOffset(byte[] b, int offset, int myLastOnDiskOffset) {
        long type = LittleEndian.getUShort(b,offset+2);
        long rlen = LittleEndian.getUInt(b,offset+4);
	 ...
	 */
	public static Records[] findChildRecords(byte[] b, int start, int len) {
		List<Records> children = new ArrayList<Records>(5);
		...
		while(pos <= (start+len-8)) {
			...
			Records r = createRecordForType(type,b,pos,8+rleni);
			if(r != null) {
				children.add(r);
			} else {
				// Records was horribly corrupt
			}
			pos += 8;
			pos += rleni;
		}
		...
		Records[] cRecords = children.toArray( new Records[children.size()] );
		return cRecords;
	}
	 ...
	 *  will return a Records object that will handle that record
	 *
	 ...
	 */
	public static Records createRecordForType(long type, byte[] b, int start, int len) {
		Records toReturn = null;
		...
		try {
			...
			java.lang.reflect.Constructor<? extends Records> con = c.getDeclaredConstructor(new Class[] { byte[].class, Integer.TYPE, Integer.TYPE });
			// Instantiate
	...
	public static Records createRecordForType(long type, byte[] b, int start, int len, int myLastOnDiskOffset) {
        Records toReturn = null;
