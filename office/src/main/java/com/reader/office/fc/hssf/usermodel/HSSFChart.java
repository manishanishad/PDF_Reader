...
import com.reader.office.fc.hssf.util.CellRangeAddress;
import com.reader.office.fc.ss.util.CellRangeAddressBase;
...
public final class HSSFChart extends HSSFSimpleShape {
	...
	private Map<SeriesTextRecord, Records> chartSeriesText = new HashMap<SeriesTextRecord, Records>();
	 ...
	 */
	private static int convetRecordsToSeriesByPostion(List<Records> records, HSSFChart chart, int seriesStart)
	{
	 ...
	 * @return
	 */
	private static int convetRecordsToText(List<Records> records, HSSFChart chart, int seriesStart)
	{
	 ...
	 * @param chart
	 */
	public static void convertRecordsToChart(List<Records> records, HSSFChart chart)
	{
...
//	        	chart.series.get(chart.series.size() - 1).setTextRecord((TextRecord)r);
            ...
            else if (r instanceof Records)
            {
                if (chart != null)
                {
                    Records record = (Records) r;
	 ...
	 * @return
	 */
	public Map<SeriesTextRecord, Records> getSeriesText()
	{
	...
	private void createAxisRecords( List<Records> records )
	{
	...
	public HSSFSeries createSeries() throws Exception {
		...
		for(final RecordBase record : seriesTemplate) {		
			...
			if (record instanceof BeginRecord) {
				...
				newRecord = (Records) ((Records)record).clone();
			}
