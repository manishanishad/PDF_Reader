...
import com.reader.office.fc.hssf.record.CellValueRecordInterface;
...
import com.reader.office.fc.hssf.record.aggregates.ChartSubstreamRecordAggregate;
...
import com.reader.office.fc.hssf.record.aggregates.RecordAggregate.PositionTrackingVisitor;
...
import com.reader.office.fc.hssf.util.ColumnInfo;
import com.reader.office.fc.hssf.util.HSSFPaneInformation;
import com.reader.office.fc.ss.util.HSSFCellRangeAddress;
import com.reader.office.fc.util.Internal;
import com.reader.office.fc.util.POILogFactory;
 ...
 * @author Jason Height (jheight at chariot dot net dot au) Clone support. DBCell & Index Records writing support
...
public final class InternalSheet
{
     ...
     * API.  Pass in an array of Records objects, the sheet number (0 based) and
    ...
    private static final class RecordCloner implements RecordVisitor
    {
        ...
        private final List<Records> _destList;

        public RecordCloner(List<Records> destList)
        {
            _destList = destList;
        }
     ...
     * can be added to a sheet. The <b>Records</b> object does not implement cloneable.
     * When adding a new record, implement a public clone method if and only if the record
     * belongs to a sheet.
     */
    public InternalSheet cloneSheet()
    {
        List<Records> clonedRecords = new ArrayList<Records>(_records.size());
        for (int i = 0; i < _records.size(); i++)
        {
            ...
            Records rec = (Records)((Records)rb).clone();
     ...
     *
     * @see com.reader.office.fc.hssf.record.DimensionsRecord
     */
    ...
    public void visitContainedRecords(RecordVisitor rv, int offset)
    {
        ...
        for (int k = 0; k < _records.size(); k++)
        {
            ...
            {
                ptv.visitRecord((Records)record);
            }
     ...
     * @see com.reader.office.fc.hssf.record.CellValueRecordInterface
     */
    ...
    /**
     * creates the DefaultRowHeight Records and sets its options to 0 and rowheight to 0xff
     */
    ...
    /**
     * creates the DefaultColWidth Records and sets it to 8
      */
     ...
     * @see com.reader.office.fc.hssf.record.DefaultColWidthRecord
     * @see com.reader.office.fc.hssf.record.ColumnInfoRecord
    ...
    /**
     * creates the Dimensions Records and sets it to bogus values (you should set this yourself
    ...
    /**
     * creates the WindowTwo Records and sets it to:  <P>
     * options        = 0x6b6 <P>
     * toprow         = 0 <P>
     * leftcol        = 0 <P>
     * headercolor    = 0x40 <P>
    ...
    */
     ...
     *
     * @see com.reader.office.fc.hssf.record.SelectionRecord
     ...
     */
    public Records findFirstRecordBySid(short sid)
    {
        ...
        return (Records)_records.get(ix);
    }
    ...
    { // TODO - remove this method
        ...
        for (int i = 0; i < max; i++)
        {
            ...
            if (!(rb instanceof Records))
            {
                continue;
            }
            Records record = (Records)rb;
    ...
    public HSSFChart getChart()
	{
    	if(sheetType == BOFRecord.TYPE_CHART)
    	{
        	...
        	if(loc >= 0)
        	{
        		List<Records> chartRecordsList = new ArrayList<Records>();
        		RecordBase record =  _records.get( loc );
                while(!(record instanceof WorksheetProtectionBlock))
                {
                    chartRecordsList.add((Records)record);
