...
import com.reader.office.fc.hslf.util.MutableByteArrayOutputStream;
import com.reader.office.fc.util.ArrayUtil;
...
public abstract class RecordContainer extends Records
{
	protected Records[] _children;
	private Boolean changingChildRecordsLock = Boolean.TRUE;

	/**
	 * Return any children
	 */
    public Records[] getChildRecords() { return _children; }
	 ...
	 */
	private int findChildLocation(Records child) {
	 ...
	 */
	private void appendChild(Records newChild) {
		synchronized(changingChildRecordsLock) {
			// Copy over, and pop the child in at the end
			Records[] nc = new Records[(_children.length + 1)];
	...
	/**
	 * Adds the given new Child Records at the given location,
	 *  shuffling everything from there on down by one
	 * @param newChild
	 * @param position
	 */
	private void addChildAt(Records newChild, int position) {
		synchronized(changingChildRecordsLock) {
	 ...
	 */
	public Records findFirstOfType(long type) {
		for(int i=0; i<_children.length; i++) {
     ...
     */
    public Records removeChild(Records ch) {
        Records rm = null;
        ArrayList<Records> lst = new ArrayList<Records>();
        for(Records r : _children) {
            if(r != ch) lst.add(r);
            else rm = r;
        }
        _children = lst.toArray(new Records[lst.size()]);
        return rm;
    }
	 ...
	 */
	public void appendChildRecord(Records newChild) {
		synchronized(changingChildRecordsLock) {
			appendChild(newChild);
		}
	}
	...
	/**
	 * Adds the given Child Records after the supplied record
	 * @param newChild
	 * @param after
	 */
	public void addChildAfter(Records newChild, Records after) {
		synchronized(changingChildRecordsLock) {
	...
	/**
	 * Adds the given Child Records before the supplied record
	 * @param newChild
	 * @param before
	 */
	public void addChildBefore(Records newChild, Records before) {
		synchronized(changingChildRecordsLock) {
			// Decide where we're going to put it
	 ...
	 */
	public void moveChildBefore(Records child, Records before) {
	 ...
	 * Moves the given Child Records to before the supplied record
	 */
	public void moveChildrenBefore(Records firstChild, int number, Records before) {
		if(number < 1) { return; }
	 ...
	 */
	public void moveChildrenAfter(Records firstChild, int number, Records after) {
		if(number < 1) { return; }
     ...
     */
    public void setChildRecord(Records[] records) {
    ...
    public static void handleParentAwareRecords(RecordContainer br) {
        ...
        for (Records record : br.getChildRecords()) {
