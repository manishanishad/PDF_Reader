...
import com.reader.office.fc.ddf.EscherBinaryTagRecord;
...
public abstract class Sheet
{
        ...
        for (int i = 0; i < wrappers.length; i++)
        {
            ...
            if (s2 != s1)
            {
                ...
                for (int j = i - 1; j >= 0; j--)
                {
                    ...
                    {
                        Records[] records = wrappers[j].getChildRecords();
     ...
     */
    protected static void findTextRuns(Records[] records, Vector found)
    {
        ...
        for (int i = 0, slwtIndex = 0; i < (records.length - 1); i++)
        {
            if (records[i] instanceof TextHeaderAtom)
            {
                ...
                if (trun != null)
                {
                    ...
                    Records[] recs = new Records[lst.size()];
                    lst.toArray(recs);
                    trun._records = recs;
     ...
     *
     * @param type  type of text, See {@link com.reader.office.fc.hslf.record.TextHeaderAtom}
     ...
     *
     * @param type  type of placeholder to search. See {@link com.reader.office.fc.hslf.record.OEPlaceholderAtom}
     * @return  <code>TextShape</code> or <code>null</code>
     */
    ...
    /**
     * Records container that holds sheet data.
